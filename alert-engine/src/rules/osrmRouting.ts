...
import { calculateHaversineDistanceMeters } from './collisionClose.js';
...
export const MINE_HAUL_NETWORK: Array<{ id: string; waypoints: Array<{ lat: number; lon: number }> }> = [
  {
    id: 'RAMP-01-CRUSHER',
    waypoints: [
...
function calculateGraphDistance(
  ...
  for (const road of MINE_HAUL_NETWORK) {
    ...
    for (let i = 0; i < road.waypoints.length; i++) {
      ...
      const d1 = calculateHaversineDistanceMeters(lat1, lon1, wp.lat, wp.lon);
      const d2 = calculateHaversineDistanceMeters(lat2, lon2, wp.lat, wp.lon);
    ...
    if (minDist1 < 100 && minDist2 < 100 && idx1 !== -1 && idx2 !== -1) {
      ...
      for (let j = startIdx; j < endIdx; j++) {
        roadDistance += calculateHaversineDistanceMeters(
          road.waypoints[j].lat,
          road.waypoints[j].lon,
          road.waypoints[j + 1].lat,
          road.waypoints[j + 1].lon
        );
      }
      return roadDistance;
    }
 ...
 */
export async function calculateRoadDistanceBetweenVehicles(
  v1: TelemetryPayload,
  v2: TelemetryPayload
): Promise<RouteDistanceResult> {
  // 1. Check Mine Haul Road Network Graph first (dedicated private pit haul roads)
  const graphDist = calculateGraphDistance(
    v1.position.lat,
    v1.position.lon,
    v2.position.lat,
    v2.position.lon
  ...
  const haversineDist = calculateHaversineDistanceMeters(
    v1.position.lat,
    v1.position.lon,
    v2.position.lat,
    v2.position.lon
