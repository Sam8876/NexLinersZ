...
export const checkOverspeeding: AlertRule = (fleet, telemetry) => {
  ...
  if (speed > speedLimit) {
    ...
    let action: 'raise' | 'escalate' | 'update' = 'raise';
    if (activeAlert) {
      if (
        (activeAlert.severity === 'medium' && (severity === 'high' || severity === 'critical')) ||
