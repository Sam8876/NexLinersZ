...
exports.checkCollisionClose = exports.calculateDistanceMeters = void 0;
exports.calculateHaversineDistanceMeters = calculateHaversineDistanceMeters;
exports.checkCollisionCloseAsync = checkCollisionCloseAsync;
const osrmRouting_js_1 = require("./osrmRouting.js");
/**
...
exports.calculateDistanceMeters = calculateHaversineDistanceMeters;
/**
 * Synchronous collision close rule using Haversine algorithm
 */
const checkCollisionClose = (fleet, telemetry) => {
    const alerts = [];
    const SAFE_FOLLOWING_THRESHOLD_M = 50.0;
    for (const [otherId, otherState] of fleet.entries()) {
        if (otherId === telemetry.vehicleId)
            continue;
        const otherTel = otherState.lastTelemetry;
        const distanceM = calculateHaversineDistanceMeters(telemetry.position.lat, telemetry.position.lon, otherTel.position.lat, otherTel.position.lon);
...
exports.checkCollisionClose = checkCollisionClose;
/**
 ...
 */
async function checkCollisionCloseAsync(fleet, telemetry) {
    const alerts = [];
    ...
    for (const [otherId, otherState] of fleet.entries()) {
        ...
        const straightLineM = calculateHaversineDistanceMeters(telemetry.position.lat, telemetry.position.lon, otherTel.position.lat, otherTel.position.lon);
        ...
        const roadResult = await (0, osrmRouting_js_1.calculateRoadDistanceBetweenVehicles)(telemetry, otherTel);
